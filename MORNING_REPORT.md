...
- **显存峰值 3.62GB,无 OOM**(预期 4-5GB,12GB 卡大富余)✅
- 每句 12-13s(5-7s 语音)≈ RTF 2.2,批量配音吞吐可接受;引擎调用方式已回写 `src/boke/synthesize.py`(CosyVoiceEngine),流水线 `--tts-engine cosyvoice3` 直接可用
...
4. **克隆试合成 A/B 试听**:`out/clone_test_1/2/3.wav`,判断像不像圆脸;顺便听多音字(重庆/银行/差强人意)读对没
