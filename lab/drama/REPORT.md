...
> 本版为票08 夜链汇总（2026-09-25），覆盖票02~票07 全部客观结果；本夜链的晨报见 `.xcheck/20260925-083546/MORNING.md`（夜链专用，与本文件合读即可晨间决策）；仓库根的 `MORNING_REPORT.md` 是 2026-09-23 采访主线夜班的交接汇报，与本研究线无关，勿混。
...
**R1 结果（部分，n=7，2026-09-25 当日完成）**：用户手机外放首听 7 题、耳机复听（答案未改），自述两种条件下均"分不出"。计分：2/7=29%，双尾二项 p=0.45（与 50% 瞎猜不可区分；29% 偏低属小样本噪声，18 题哈希完整性 0 异常排除映射错误）。**初判（provisional）：分离对照臂（F1 修正版）对听者透明——分离+回混不穿帮**，含客观差异 normal 档（对白+2dB）题目亦不可闻。用户选择中止余下 11 题；正式判定（n=16~20）未做，如需补测可用 serve_abx.py 随时恢复（试次与答案已存盘）。机器可读结果：results/abx/R1_partial_result.json。


**克隆臂结果（当日补充，n=5）**：原声 vs CosyVoice3 同文复刻，用户盲判——可辨 4/5=80%（瞎猜 50%，p=0.38 仅方向性）；相似度均分 2.8/5，**按参考音质量分层显著**：博主干净长参考（25s+逐字稿，主线标准链）zh 4.0/5 vs Sintel 分离 stem 短参考（3s，情绪表演语音）en 2.0/5。结论（provisional）：克隆可辨但"像不像"由参考音质量决定；**研究瓶颈从"背景分离/回混"（已证透明）转移到"克隆参考音质量"**——影视场景参考音须从多段干净独白聚合，分离 stem 直接当参考音不可行（与调研05 §5.5 预判一致，得实证）。机器可读：results/abx/clone_arm_result.json。
